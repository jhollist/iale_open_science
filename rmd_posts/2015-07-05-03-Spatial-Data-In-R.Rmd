...
libs<-c("rgdal","sp","ggplot2","maptools","ggmap","raster","rgeos") #list of packages to load
...
3. `leafletR`: a new interactive map package.  Works great for unprojected spatial data.  This package is a great product that will certainly increase in functionality. 
...
#create SpatialPolygonsDataFrame Object
  LocPolyDF<- SpatialPolygonsDataFrame(LocPoly,data.frame(ID=1,Info='PolygonAroundPDXhilton')) #add field "Info" as an attribute to the polygon.
  
#write the spatial dataframe objects to shapefiles
  writeOGR(LocPtDF,'../files','LocPt', driver="ESRI Shapefile",overwrite_layer=TRUE)
  writeOGR(LocPolyDF,'../files','LocPoly', driver="ESRI Shapefile",overwrite_layer=TRUE)

#create KML file of locations with the maptools package 
  #point
    kmlPoints(LocPtDF, 
              kmlfile='../files/LocPt.kml', 
              name="Hilton", 
...
```{r readSpatialData} 
# Using rgdal command - readOGR - we now read in the shapefiles for our current location in Portland.
  #change working directory to file location
    setwd('../files/')  #this is a relative path to the file location-you will need to change this
...
```{r dataView}
class(Pt) #class info for object

typeof(Pt) #storage type

str(Pt) # data structure for object

summary(Pt) #summary info for object
...
4. (Optional) Set the working directory in R (setwd()) to the location of the files.
...
```{r leafletPipe, message=FALSE, eval=FALSE}  
...
- Jeff Hollister is  developing an interactive map viewer that does work with projected data.  His package is called quickmapper and Jeff demonstrate it later if there is time.
...
```{r dataViewRaster}
NLCD<-raster("../files/NLCDpdx.tif")  #simple read a raster image with the raster package.
